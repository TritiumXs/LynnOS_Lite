#!/usr/bin/python3
'''
Get struct definitions in DWARF
'''
...
def get_offset_in_cu_by_type(cu, type_value):
    ...
    for child in cu_die.children:
        ...
        if "type" in child.attr_dict:
            #if type_value == child.attr["type"]
            tmp = child.attr_dict["type"].value
            if tmp == type_value:
                return child.offset
    ...
    if die.tag == DW_TAG.enumeration_type:
        ...
        if name is None:
            ...
            if written[(die.tag, name)] != WRITTEN_FINAL:
                ...
                if name in typesWanted:
                    typesWanted.remove(name)
        ...
        if written[(die.tag, name)] != WRITTEN_FINAL:
            reftype = ref(name)
            #get_offset_in_cu_by_type(cu, reftype)
            if type(reftype.type) == c_ast.Struct and reftype.type.decls == None:
                offset = die.attr_dict["type"].value
                to_c_process(by_offset[offset], by_offset, names, rv, written)
        ...
        reftype = ref(name)
        if type(reftype.type) == c_ast.Struct and reftype.type.decls == None:
            typesWanted.insert(0, reftype.type.name)
        ...
        if get_flag(die, 'declaration', False):
            ...
            for enumval in die.children:
                ...
                reftype = ref(ename)
                if (type(reftype.type) == c_ast.Struct or type(reftype.type) == c_ast.Union) and reftype.type.decls == None:
                    offset = enumval.attr_dict['type'].value
                    to_c_process(by_offset[offset], by_offset, names, rv, written)
                items.append(c_ast.Decl(ename,[],[],[], reftype, None,
        ...
        for val in die.children:
            if val.tag == DW_TAG.subrange_type:
                tmp_value = get_int(val, 'upper_bound')
                if tmp_value is not None:
                    count = tmp_value + 1 # count is upper_bound + 1
                tmp_value = get_int(val, 'count')
                if tmp_value is not None:
                    count = tmp_value
...
def parse_dwarf(infile, cuname):
    ...
    if cuname:
        ...
        for cu in dwarf.info.cus:
            ...
            tmp = process_compile_unit(dwarf, cu, written)
            tmp.extend(statements)
            statements = tmp
    return statements
...
def main():
    ...
    with open("LosTaskCB.h", mode="a") as fp:
        fp.write(CGenerator().visit(ast))
