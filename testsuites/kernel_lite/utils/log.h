...
extern "C" {
...
#ifdef DEBUG
    const static float SYS_NS_PER_S = 1000 * 1000 * 1000;
    ...
    #define LOGD(format, ...) printf(stdout, "[%.06f] " format "\n", GetCurTime(), ##__VA_ARGS__)
    #define LOG(format, ...)  printf(stdout, "[%.06f] " format "\n", GetCurTime(), ##__VA_ARGS__)
#else
    #define LOGD(...)
    #define LOG(format, ...)  printf(format "\n", ##__VA_ARGS__)
#endif

#define LOGE(format, ...)  printf(stdout,  "\n%s:%d: " format "\n", __FILE__, __LINE__, ##__VA_ARGS__)
